...
for i in range(len(requestedSizes):
    if len(requestedSizes = 0)):
        ...
        if len(TShirtSizes) < len(requestedSizes):
            ...
            else if requestedSizes[i] == "M" and TShirtSizes[i].contains("L"):
               TShirtSizes.pop(i)
               requestedSizes.pop(i)
            else if (TShirtSizes[i].count("X") > requestedSizes[i].count("X") and TShirtSizes[i].contains("S")) or
                    (TShirtSizes[i].count("X") > requestedSizes[i].count("X") and TShirtSizes[i].contains("L")):
               TShirtSizes.pop(i)
               requestedSizes.pop(i)
            else:
               print("No")
